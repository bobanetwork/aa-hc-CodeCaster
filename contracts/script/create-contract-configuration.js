...
const TRANSLATOR_TOKEN_CONTRACT = args[4];
const BACKEND_URL = args[5] ?? 'https://codecaster-backend.onrender.com/hc'; // use public backend by default

console.log('HCH = ', HC_HELPER_ADDR)
console.log('HA = ', HYBRID_ACCOUNT);
console.log('TTP = ', TRANSLATOR_TOKEN_CONTRACT);
console.log('BE = ', BACKEND_URL)
console.log('RPC_URL = ', RPC_URL)

const signer = new ethers.Wallet(PRIVATE_KEY, new ethers.JsonRpcProvider(RPC_URL));

if (!HC_HELPER_ADDR || !HYBRID_ACCOUNT || !TRANSLATOR_TOKEN_CONTRACT || !BACKEND_URL) {
...
async function main() {
    ...
    try {
        ...
        await permitCaller(TRANSLATOR_TOKEN_CONTRACT);
        console.log('DONE')
    } catch (e) {
